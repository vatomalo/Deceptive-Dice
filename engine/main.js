...
// + BIOME + RUN SEED (ADDED, MINIMAL / SAFE)
// =======================================================
...
// ===================================================
// RUN SEED (Deterministic randomness per run/session)
// ===================================================
if (typeof window.RunSeed !== "number") {
  window.RunSeed = (window.crypto?.getRandomValues)
    ? crypto.getRandomValues(new Uint32Array(1))[0]
    : (Date.now() >>> 0);
  console.log("[RunSeed]", window.RunSeed);
}

// ===================================================
// BIOME SYSTEM (optional; used by ParallaxBG / Decor if they listen)
// ===================================================
window.Biomes = window.Biomes || {
  current: null,
  set(id) {
    if (!id) id = "plains";
    ...
    window.dispatchEvent(new CustomEvent("BIOME_CHANGED", { detail: { id } }));
  }
};

// Stage -> biome mapping (edit freely as you add stages)
function biomeForStage(stage) {
  const map = {
    default_stage: "plains",
    house_stage: "plains",
    forest_stage: "forest",
    sakura_stage: "sakura",
    snow_stage: "snow",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ---------------------------------------------------
  // Canvas click handler #1 (stance tap region + xbar)
  // ---------------------------------------------------
  canvas.addEventListener("click", (e) => {
    ...
    // Touch stance change: Tap in the top-left HUD area (nameplate/sword)
    const stanceRegionYMax = 110;
  ...
  // ---------------------------------------------------
  // Canvas click handler #2 (fullscreen / materia / xbar)
...
async function startGame() {
  ...
  // ADDED: set biome at start (safe)
  try {
    window.Biomes.set(biomeForStage(stageSelect));
  } catch (e) {
    console.warn("Biome init failed (safe to ignore):", e);
  }
  ...
  // Palette + weather
  PaletteSystem.start("day");
  PaletteSystem.timeScale = 0.5;
