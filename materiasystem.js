...
max: false   // enables MAX gambit
    ...
    (function () {
        ...
        // =======================================================
        // Materia Equip System v1 (Slots + Inventory)
        // =======================================================

        // Inventory = what you own (strings)
        window.MateriaInventory = window.MateriaInventory || [];

        // Slots = equipped materia keys (strings or null)
        window.MateriaSlots = window.MateriaSlots || {
            maxSlots: 2,              // start small: only 2 selectable
            equipped: [null, null],   // ["crit", "regen"] etc
        };

        // Convenience: build a boolean map for your existing code
        // (so code that checks Materia.crit still works)
        window.Materia = window.Materia || {};

        window.rebuildMateriaFlags = function () {
            // reset all flags
            for (const k in window.Materia) window.Materia[k] = false;

            // enable equipped
            for (const key of (window.MateriaSlots?.equipped || [])) {
                if (!key) continue;
                window.Materia[key] = true;
            }

            // derived: MAX gambit enabled if you have certain combo
            // v1: require BOTH slots filled (2/2) to enable max gambit
            // (later: require a specific key like "maxcore")
            window.Materia.maxGambit = (window.MateriaSlots.equipped.filter(Boolean).length >= window.MateriaSlots.maxSlots);

            console.log("[MATERIA] Equipped:", window.MateriaSlots.equipped, "maxGambit:", window.Materia.maxGambit);
        };

        // Ensure base keys exist (so reset loop is stable)
        ["crit", "regen", "speed", "barrier", "counter", "thorns", "poison", "maxGambit"].forEach(k => {
            if (typeof window.Materia[k] === "undefined") window.Materia[k] = false;
        });
